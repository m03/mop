...
type Profile struct {
	...
	// Time interval to refresh market data.
	MarketRefresh int `json:"MarketRefresh"`
	// Time interval to refresh stock quotes.
	QuotesRefresh int `json:"QuotesRefresh"`
	// Column number by which we sort stock quotes.
	SortColumn int `json:"SortColumn"`
	// True when sort order is ascending.
	Ascending bool `json:"Ascending"`
	// True when stocks are grouped by advancing/declining.
	Grouped bool `json:"Grouped"`
	// Filter in human form
	Filter string `json:"Filter"`
	// The filter as a govaluate expression
	FilterExpression *govaluate.EvaluableExpression
	// Stores selected column number when the column editor is active.
	SelectedColumn int
	// Colors of the displayed data.
	Color *Color `json:"Color"`
	// Path to the file in which the configuration is stored
	filename string
}
